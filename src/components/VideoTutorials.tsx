...
const VideoGrid: React.FC<{
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredVideos.map(video => (
            ...
            >
                <div className="relative bg-black/10 dark:bg-black/30 h-32 flex items-center justify-center group-hover:bg-black/20 dark:group-hover:bg-black/40 transition-colors">
                    <PlayIcon className="w-12 h-12 text-white opacity-70 group-hover:opacity-100 transition-opacity" />
                    <div className="absolute top-2 right-2 bg-black/70 px-2 py-1 rounded text-white text-xs font-semibold">
                        {video.duration} min
                    </div>
                    <div className="absolute bottom-2 right-2 bg-blue-600 px-2 py-1 rounded text-white text-xs font-semibold">
                ...
                <div className="p-4 space-y-3">
                    ...
                    <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                        <p className="text-xs text-gray-500 dark:text-gray-500">
                            by <span className="font-semibold text-gray-700 dark:text-gray-300">{video.creator}</span>
                        </p>
                    </div>
                </div>
            </button>
...
export const VideoTutorials: React.FC = () => {
    ...
    const levels = [
        { id: 'all', name: 'All Levels', color: 'bg-gray-100 dark:bg-gray-700' },
        { id: 'basic', name: 'Beginner', color: 'bg-green-100 dark:bg-green-900' },
        { id: 'intermediate', name: 'Intermediate', color: 'bg-blue-100 dark:bg-blue-900' },
        { id: 'advanced', name: 'Advanced', color: 'bg-orange-100 dark:bg-orange-900' },
        { id: 'expert', name: 'Expert', color: 'bg-red-100 dark:bg-red-900' },
    ] as const;
        ...
        <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex flex-wrap gap-2">
                    {levels.map(level => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg font-medium transition-all ${selectedLevel === level.id
                                    ? `${level.color} ring-2 ring-offset-2 dark:ring-offset-gray-900 ring-blue-500`
                                    : `${level.color} hover:ring-2 hover:ring-offset-2 dark:hover:ring-offset-gray-900 hover:ring-blue-300 text-gray-700 dark:text-gray-200`
                                }`}
                        >
                            {level.name}
                        </button>
                ...
                {filteredVideos.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                            {searchQuery ? t('videoTutorials.noResults') : 'No tutorials available for this level.'}
                        </p>
                    </div>
                ) : (
                    <VideoGrid
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                <p className="text-sm text-amber-800 dark:text-amber-200">
                    <span className="font-semibold">📜 Legal Notice:</span> {t('videoTutorials.legalNotice')}
                </p>
            </div>
        </div>
    );
